(function ($, themes) {
    ...
    // SVG from https://material.io/tools/icons/?icon=code&style=baseline
    var CUSTOM_CORNER = {
    ...
    function githubCornerTemplate(theme){
        var div = document.createElement('DIV');
        var option = getDisplayFlairCornerOption();
        if (CUSTOM_CORNER.hasOwnProperty(option)) {
            div.innerHTML = CUSTOM_CORNER[option];
        } else {
    ...
    function getDisplayFlairCornerOption() {
        return $("#display-flair-corner-option :selected").val();
    }
